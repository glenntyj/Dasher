...
public static class DBUtl
{
    ...
    {
        EscStringParameters(list);
        string escSQL = String.Format(sql, list);

        DataTable dt = new DataTable();
        using (MySqlConnection dbConn = new MySqlConnection(DB_CONNECTION))
        using (MySqlDataAdapter dAdptr = new MySqlDataAdapter(escSQL, dbConn))
        {
            try
            {
                dAdptr.Fill(dt);
    ...
    private static void EscStringParameters(object[] list)
    {
        if (list != null)
